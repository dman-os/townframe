...
const ghjk = file({
	...
	enableRuntimes: true,
	allowedBuildDeps: [installs.node, installs.rust, installs.py],
});
...
ghjk.task("kanidm-recover", async ($) => {
...
ghjk.task("dev-gran", ($) => $`trunk serve`, {
...
ghjk.task(
	"compose-down",
	async ($) =>
		$.raw`${DOCKER_CMD} compose ${($.argv.length
			? $.argv
			: await allProfiles($)
		)
			.map((prof) => `--profile ${prof}`)
...
ghjk.task("db-mig", { dependsOn: ["db-mig-btress"] });

ghjk.task("db-mig-btress", ($) =>
...
ghjk.task("db-seed", { dependsOn: ["db-seed-btress"] });
...
ghjk.task("test-rust", ($) => $`cargo nextest run -p tests_http`, {
	desc: "Run rust tests",
	vars: {
...
ghjk.task("gradle", ($) => $`./src/daybook_compose/gradlew ${$.argv}`);

ghjk.task("build-a-dayb", ($) => $`./gradlew installDebug`, {
...
ghjk.task("dev-scrcpy", ($) => $`scrcpy --no-audio -S`, {
...
ghjk.task(
	...
	async ($) => {
		...
		await $`wac plug --plug btress_api.wasm btress_http.wasm -o btress_http_plugged.wasm`.cwd(
...
ghjk.task(
	...
	async ($) => {
		...
		await $`wac plug --plug daybook_api.wasm daybook_http.wasm -o daybook_http_plugged.wasm`.cwd(
			"./target/wasm32-wasip2/debug/",
		);
	},
	{},
