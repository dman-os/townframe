...
async fn tier9_unauthorized_peer_no_plaintext_leak() -> crate::Res<()> {
    ...
    let intruder_sync = intruder_owner_conn.sync_doc_with_peer(doc_id, None).await;
    match intruder_sync {
        Ok(()) => {
...
async fn tier9_missing_doc_sync_returns_unauthorized() -> crate::Res<()> {
    ...
    let err = pair
        .left_conn()
        .sync_doc_with_peer(fake_doc_id, None)
        .await
...
async fn tier9_duplicate_concurrent_sync_converges() -> crate::Res<()> {
    ...
    let (r1, r2) = tokio::join!(
        pair.right_conn().sync_doc_with_peer(doc_id, None),
        pair.right_conn().sync_doc_with_peer(doc_id, None),
    );
