...
impl SqliteKvStore {
    ...
    pub async fn new(db_pool: sqlx::SqlitePool, table_name: impl Into<String>) -> Res<Self> {
        ...
        Ok(Self { db_pool, table_name })
    }
}

// Implement KvStore for Arc<SqliteKvStore> to match the pattern used by DHashMap
#[async_trait]
impl KvStore for Arc<SqliteKvStore> {
    async fn get(&self, key: &[u8]) -> Res<Option<Arc<[u8]>>> {
        let row = sqlx::query_scalar::<_, Vec<u8>>(
            &format!(r#"SELECT value FROM "{}" WHERE key = ?1"#, self.table_name),
        )
        .bind(key)
    ...
    async fn del(&self, key: &[u8]) -> Res<Option<Arc<[u8]>>> {
        ...
        sqlx::query(&format!(r#"DELETE FROM "{}" WHERE key = ?1"#, self.table_name))
            .bind(key)
            .execute(&self.db_pool)
            .await?;
