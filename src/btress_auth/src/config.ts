...
var allEnvs: Record<string, string>;
export function getEnvOptional(key: string) {
  if (!allEnvs) {
    allEnvs = Object.fromEntries(getEnvironment());
    console.log(allEnvs, getEnvironment());
  }
  return allEnvs[key];
}

export function getEnv(key: string) {
  return assertNotNull(getEnvOptional(key), key);
}
