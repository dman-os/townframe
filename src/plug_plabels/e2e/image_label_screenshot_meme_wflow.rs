...
async fn long_af_test_image_label_fallback_multi_label_screenshot_meme() -> Res<()> {
    ...
    for _ in 0..1200 {
        if let Some(doc) = test_cx
            ...
            .await?
            && doc.facets.contains_key(&embedding_key)
            && doc.facets.contains_key(&pseudo_label_key)
        {
