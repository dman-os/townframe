...
pub mod contract {
        ...
        S: PartStore<Sendable> + Sync,
    {
        ...
        store
            .add_obj_to_parts(obj_id.clone(), std::slice::from_ref(&part_id))
            .await;
        assert_eq!(
        ...
        store
            .remove_obj_from_part(obj_id.clone(), part_id.clone())
            .await;
    ...
    pub async fn assert_add_obj_to_parts_is_idempotent<S>(
        store: &S,
        part_id: PartKey,
        obj_id: ObjKey,
    ) where
        ...
        S: PartStore<Sendable> + Sync,
    {
        ...
        store
            .set_peer_part_cursor(peer_id.clone(), part_id.clone(), 17)
            .await;
        assert_eq!(
            store
                .get_peer_part_cursor(peer_id.clone(), part_id.clone())
        ...
        store
            .set_peer_part_cursor(peer_id.clone(), part_id.clone(), 23)
            .await;
