...
pub mod contract {
        ...
        S: PartStore<Sendable> + Sync,
    {
        ...
        store.upsert_obj(obj_id.clone(), &payload_a).await;
        store.add_obj_to_parts(obj_id.clone(), std::slice::from_ref(&part_id)).await;
        assert_eq!(store.obj_payload(obj_id.clone()).await, Some(payload_a.clone()));
        assert_eq!(store.obj_parts(obj_id.clone()).await, vec![part_id.clone()]);
        assert_eq!(store.member_count(part_id.clone()).await, 1);

        store.upsert_obj(obj_id.clone(), &payload_b).await;
        store.add_obj_to_parts(obj_id.clone(), std::slice::from_ref(&part_id)).await;
        assert_eq!(store.obj_payload(obj_id.clone()).await, Some(payload_b.clone()));
        assert_eq!(store.obj_parts(obj_id.clone()).await, vec![part_id.clone()]);
        assert_eq!(store.member_count(part_id.clone()).await, 1);

        store.remove_obj_from_part(obj_id.clone(), part_id.clone()).await;
        assert_eq!(store.obj_payload(obj_id.clone()).await, None);
        assert_eq!(store.obj_parts(obj_id.clone()).await, Vec::<PartKey>::new());
        assert_eq!(store.member_count(part_id.clone()).await, 0);

        store.upsert_obj(obj_id.clone(), &payload_c).await;
        store.add_obj_to_parts(obj_id.clone(), std::slice::from_ref(&part_id)).await;
        assert_eq!(store.obj_payload(obj_id.clone()).await, Some(payload_c.clone()));
        assert_eq!(store.obj_parts(obj_id).await, vec![part_id.clone()]);
        ...
        S: PartStore<Sendable> + Sync,
    {
        ...
        store.upsert_obj(obj_id.clone(), &payload).await;
        store.add_obj_to_parts(obj_id.clone(), std::slice::from_ref(&part_id)).await;
        store.add_obj_to_parts(obj_id.clone(), std::slice::from_ref(&part_id)).await;
    ...
    where
        S: PartStore<Sendable> + Sync,
    {
        assert_eq!(store.get_peer_part_cursor(peer_id.clone(), part_id.clone()).await, 0);
        store.set_peer_part_cursor(peer_id.clone(), part_id.clone(), 17).await;
        assert_eq!(store.get_peer_part_cursor(peer_id.clone(), part_id.clone()).await, 17);
        store.set_peer_part_cursor(peer_id.clone(), part_id.clone(), 23).await;
