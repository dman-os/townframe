...
// DocPatch is generated by struct_patch from Doc in daybook_types
    ...
    pub async fn get_at_heads(&self, id: &DocId, heads: &ChangeHashSet) -> Res<Option<Arc<Doc>>> {
        ...
        let (doc, heads) = tokio::task::block_in_place(move || {
            handle.with_document(move |doc| {
                ...
                let am_doc: daybook_types::automerge::Doc = autosurgeon::hydrate(&version).wrap_err("error hydrating")?;
                let root_doc: Doc = am_doc.to_root();
                    ...
                    let result = am_doc.transact(move |tx| {
                        match cache.get_mut(&id_clone) {
                            Some(mut entry) => {
                                ...
                                doc.apply(patch);
                                // Hydrate as automerge Doc, then convert to root Doc
                                use daybook_types::AutomergeDoc as AmDoc;
                                let am_doc: AmDoc = doc.into();
                                autosurgeon::reconcile(tx, &am_doc).wrap_err("error reconciling")?;
                                // Convert back to root Doc
                            ...
                            None => {
                                // Hydrate as automerge Doc, then convert to root Doc
                                let am_doc: daybook_types::automerge::Doc = autosurgeon::hydrate(tx).wrap_err("error hydrating")?;
                                let mut doc: Doc = am_doc.to_root();
                                doc.apply(patch);
                                // Convert back to automerge Doc for reconciliation
                                let am_doc = daybook_types::automerge::Doc::to_automerge(doc);
                                autosurgeon::reconcile(tx, &am_doc).wrap_err("error reconciling")?;
                                // Convert back to root Doc
                                let root_doc: Doc = am_doc.to_root();
    ...
    async fn smoke() -> Res<()> {
        ...
        let new_doc_id = client_repo
            .add(Doc {
                ...
                content: daybook_types::DocContent::Text("Hello, world!".into()),
                props: vec![],
            })
            .await?;
