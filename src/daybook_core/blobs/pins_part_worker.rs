...
use daybook_types::doc::{BranchId, ChangeHashSet, DocId, WellKnownFacetTag};
...
async fn run_blob_pins_part_task(
    ...
    let Preparation::Ready(prepared) = ctx.prepare_facet_deltas(&drawer, &task.deltas).await?;
...
impl Worker {
    ...
    async fn run_facet_machine(
        ...
        let mut walker =
            ConcurrentDeltaWalker::open(reader, state.clone(), |entry: &FacetDelta| {
                blob_pins_part_key(&entry.key.branch_id)
            })
            .await
