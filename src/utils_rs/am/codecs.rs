...
impl autosurgeon::Hydrate for AutosurgeonJson {
    ...
    fn hydrate_scalar(s: std::borrow::Cow<'_, automerge::ScalarValue>) -> Result<Self, autosurgeon::HydrateError> {
    ...
    fn copy_object_to_doc(
        ...
        match obj_type {
                ...
                for item in source.map_range(source_obj.clone(), ..) {
                    ...
                    match source.get(source_obj, prop)? {
                        Some((automerge::Value::Object(inner_type), id)) => {
                            let new_id = target_tx.put_object(&target_obj, automerge::Prop::Map(key.clone()), inner_type)?;
                            copy_object_to_doc(source, &id, target_tx, new_id, inner_type)?;
                        }
                        Some((automerge::Value::Scalar(s), _)) => {
                            target_tx.put(&target_obj, automerge::Prop::Map(key.clone()), s.as_ref().clone())?;
                        }
                        None => {}
                    }
                }
            ...
            automerge::ObjType::Text => {
                ...
                let text_obj = target_tx.put_object(&target_obj, automerge::Prop::Map("_text".to_string()), automerge::ObjType::Text)?;
