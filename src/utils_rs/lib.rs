...
// unwrap cases
// For use them in `expect` or `wrap_err` or `warn!`
// Their primary utility is to make code easier to parse compared
// to unwraps or bespoke sentences describing a error cases
pub mod expect_tags {
    pub const ERROR_CANCELLED: &str = "cancel token was lit";
    // This is for cases where unreachable! is not suitable.
    ...
    // Spawning tasks or doing work on blocking threads can return errors
    // when tokio is shutting down.
