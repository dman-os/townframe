...
Integration with iroh-blobs is deferred to an implementation ADR. The intended design: iroh-blobs serves verified byte ranges using a BLAKE3 bao outboard alongside the data. For a virtual ciphertext, the outboard is computed during the §11 creation pass — the same streaming pass that produces `representation.digest` — and cached in local `VirtualBlob` state, so serving does not recompute the hash tree. Serving a virtual representation through iroh-blobs requires a provider path that sources bytes and outboard from an arbitrary `(outboard, AsyncRead)` pair rather than from the store; this requires a targeted modification to `iroh-blobs`, maintained on a fork. A SQLite/object-store iroh-blobs backend (for relay-grade durability and replication) is a separate future addition on the same fork.
