...
import {
  ...
  readGhVars,
} from "https://raw.githubusercontent.com/metatypedev/ghjk/v0.3.1-rc.2/src/sys_deno/ports/ghrel.ts"
...
export class Port extends PortBase {
  ...
  override async download(args: DownloadArgs) {
    ...
    const targets = conf.targets ? conf.targets.map(str => `-t ${str}`).join(" ") : "";
    await $.raw`"${
